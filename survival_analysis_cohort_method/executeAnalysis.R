...
analysisName_base = "surv_analysis"
analysisName <- paste0(analysisName_base, ".json")
results_schema_name <- paste0("res_", analysisName_base, "_amit_100pct_10")

getDuckDbConnectionDetails <- function(dbFilePath) {
  connectionDetails <- createConnectionDetails(
    dbms = "duckdb",
    server = "/Users/amit.sharma/Downloads/database-1M_filtered.duckdb"
  )
...
# connectionDetails <- getDuckDbConnectionDetails()
...
port   <- 41192 
pathToDriver <- "~/Documents/Projects"  # e.g., "/path/to/your/jdbc/driver"
# Create connection details
connectionDetails <- createConnectionDetails(
  dbms = dbms,
  server = server,
